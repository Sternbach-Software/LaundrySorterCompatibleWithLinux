...
/**
 * Library of Kotlin utility functions. Version: 2.0.3
 * */
@Suppress("UNUSED")
    ...
    inline fun <reified T> sort(
        ...
        workingList.sortWith(
            PRIVATEgetComparator(
                ascending,
                PRIVATEgetPropertyToSortBy<T>(sortCriteria[0]),
                sortCriteria,
                size.second
            )
        )
...
* */
    ...
    *  val mutableListOf = mutableListOf(true, true, true)
    ...
    fun File.performWrite(action: (BufferedWriter) -> Unit) {
        ...
        action(writer)
        writer.close()
    }//TODO make version to return the result of the action lambda

    /**
     * Performs an on a file by passing the file's [bufferedReader] to [action] and closing it after the action is done
     * */
    fun File.performRead(action: (BufferedReader) -> Unit) {
        val reader = bufferedReader()
        action(reader)
        reader.close()
    }//TODO make a version to return the result of the read (the contents of the file)
